function tangent_faces
% TANGENT_FACES  Compute tangent space of faces

% Load Viola-Jones data
if exist('data') ~= 1
  fprintf('Loading data\n') ;
  global data ;
  data=load('data/vjf') ;
  y = data.Labels ;
  %x = (data.Data - mean(data.Data(:)))' ;
  mf = mean(data.Data,2);
  sf = std(data.Data,0,2);
  sf = sf+1*(sf==0);
  x = ((data.Data - mf*ones(1,size(data.Data,2)))./(sf*ones(1,size(data.Data,2))))';
...
dsigma(5) = .2;%0.1 ; % std dev of contrast
...
for i=1:N
  ...
  if mod(i,1000)==0
    fprintf('preproc: bri: %6d of %6d (%.3f %%).\n', i,N,i/N*100) ;
  end
end
