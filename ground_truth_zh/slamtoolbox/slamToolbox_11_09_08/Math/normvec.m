...
%
%   [NV,NV_v] = NORMVEC(V,method) , with method ~= 0, uses a scalar
%   diagonal matrix as Jacobian, meaning that the vector has been just
%   scaled to length one by a scalar factor equal to its norm. The Jacobian
%   is therefore: 
%       NV_v = eye(length(v))/norm(v).
