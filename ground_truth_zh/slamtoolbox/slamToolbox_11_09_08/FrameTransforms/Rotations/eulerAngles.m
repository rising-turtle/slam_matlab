...
%
%   Beware that the [roll, pitch, yaw] terminology only makes linguistical
%   sense if the Z-axis is vertical: 
%       - For robots and terrestrial vehicles we use FLU (X-front, Y-left,
%       Z-up). Positive pitch makes the nose sink.
%       - Aircraft use to specify attitude as FRD (X-front, Y-right,
%       Z-down). Positive pitch makes the nose rise.
%       - Cameras are usually RDF (X-right, Y-down, Z-front) and the
%       mention [roll, pitch, yaw] may be confusionnary for them: positive
%       roll makes the nose (ie. the camera axis) rise.
%
