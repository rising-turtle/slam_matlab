    ...
    if arg.Extrapolation
        ...
        if k>2 && theta(k+1) < 10 && theta(k) < 10
            ...
            if dv ~= 0
                ...
                switch arg.Matching
                    case 'bruteForce'
                        [xx, mindist] = match_bruteForce(q,pt);
                    case 'Delaunay'
                        [xx, mindist] = match_Delaunay(q,pt,DT);
                    case 'kDtree'
                        [xx, mindist] = match_kDtree(q,pt,kdOBJ);
                end
...
function [match mindist] = match_kDtree(xx, p, kdOBJ)
...
[U,xx,V] = svd(N); % singular value decomposition
...
%     [V,D] = eig(P);
%     
%     [xx, idx] = min(diag(D)); % choses the smallest eigenvalue
%     
