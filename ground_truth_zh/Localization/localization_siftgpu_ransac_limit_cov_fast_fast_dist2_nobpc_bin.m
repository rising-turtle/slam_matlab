...
function [phi, theta, psi, trans, error, elapsed, match_num, feature_points, pose_std] = localization_siftgpu_ransac_limit_cov_fast_fast_dist2_nobpc_bin(image_name, data_name, filter_name, boarder_cut_off, ransac_iteration_limit, valid_ransac, scale, value_type, dm, inc, j, sframe, sequence_data, is_10M_data, dis)
...
if first_cframe > 0
    if check_stored_visual_feature(data_name, dm, cframe, sequence_data, image_name) == 0
        ...
        if strcmp(data_name, 'kinect_tum')
            ...
            ld_error=0;
            [img1, x1, y1, z1, c1, elapsed_pre, ld_error] = LoadSR_no_bpc_time_single_binary(data_name, filter_name, boarder_cut_off, dm, first_cframe, scale, value_type);
        end
        
        if ld_error < 0
            fprintf('No image is loaded !!\n');
            error=ld_error;
...
%         else
            ...
            if sift_threshold == 0
                %[frm1, des1] = sift(img1);
                [des1, frm1]=siftgpu_rawdata(img1);
            else
...
%         end
    ...
    if dis==1 && j == 1
        figure(1);
        subplot(2,2,3);
        imagesc(img1); colormap(gray); axis image;
        set(gca,'XTick',[]);
        set(gca,'YTick',[]);
        title(sprintf('Frame %d', first_cframe));
        drawnow;
    end
...
if sequence_data == true
    ...
    cframe = sframe;   % Generate constraints
else
    dm=dm+inc;
    cframe = j;
end
...
if check_stored_visual_feature(data_name, dm, cframe, sequence_data, image_name) == 0
    ...
    if strcmp(data_name, 'kinect_tum')
        ...
        ld_error2=0;
        [img2, x2, y2, z2, c2, elapsed_pre2, ld_error2] = LoadSR_no_bpc_time_single_binary(data_name, filter_name, boarder_cut_off, dm, second_cframe, scale, value_type);
    end
...
%     else
        ...
        if sift_threshold == 0
            %[frm2, des2] = sift(img2);
            [des2, frm2]=siftgpu_rawdata(img2);
        else
...
if dis==1
    figure(1);
    subplot(2,2,1);
    imagesc(img2); colormap(gray); axis image;
    set(gca,'XTick',[]);
    set(gca,'YTick',[]);
    title(sprintf('Frame %d', second_cframe));
    drawnow;
end

if first_cframe <= 0
    fprintf('First is loaded !!\n');
    error=4;
    phi=0.0; theta=0.0; psi=0.0; trans=[0.0; 0.0; 0.0];
    elapsed_ransac = 0.0;
    elapsed_svd = 0.0;
...
%     end
    ...
    if strcmp(data_name, 'kinect_tum')
        ...
        for i=1:pnum
            ...
            matched_pix1=frm1(:, frm1_index);     COL1=round(matched_pix1(1))+1;     ROW1=round(matched_pix1(2))+1;
            matched_pix2=frm2(:, frm2_index);     COL2=round(matched_pix2(1))+1;     ROW2=round(matched_pix2(2))+1;
            if COL1 <= 176 && COL2 <= 176 && ROW1 <= 144 && ROW2 <= 144
