...
function [phi, theta, psi, trans, error, elapsed, match_num, feature_points, pose_std] = localization_sift_ransac_limit_icp2_cov_f_f_dist2_loadicp_nobpc(image_name, data_name, filter_name, boarder_cut_off, ransac_iteration_limit, valid_ransac, scale, value_type, dm, inc, j, sframe, icp_mode, is_10M_data, dis)
...
%Read first data set
if strcmp(data_name, 'm') || strcmp(data_name, 'etas') || strcmp(data_name, 'loops2') || strcmp(data_name, 'sparse_feature') || strcmp(data_name, 'swing') || strcmp(data_name, 'swing2') || strcmp(data_name, 'motive') || strcmp(data_name, 'map')% Dynamic data
    cframe = sframe;
    sequence_data = true;
else
    cframe = j;
...
if check_stored_icp_pose(data_name, dm, first_cframe, second_cframe, icp_mode, sequence_data)
    
    [phi_icp, theta_icp, psi_icp, trans_icp, rmse_icp, match_num, elapsed_icp, sta_icp, error, pose_std] = load_icp_pose(data_name, dm, first_cframe, second_cframe, icp_mode, sequence_data);
    ...
    if strcmp(icp_mode, 'icp_ch')
        ...
        %[phi_icp, theta_icp, psi_icp, trans_icp, rmse_icp, match_num, elapsed_icp, sta_icp, error, pose_std] = vro_icp_6_cov_dynamic(op_pset1, op_pset2, rot, trans, x1, y1, z1, img1, c1, x2, y2, z2, img2, c2);
    end
    
    if error == 0 && sta_icp == 1
        save_icp_pose(data_name, dm, first_cframe, second_cframe, icp_mode, sequence_data, phi_icp, theta_icp, psi_icp, trans_icp, rmse_icp, match_num, elapsed_icp, sta_icp, error, pose_std);
    end
end
