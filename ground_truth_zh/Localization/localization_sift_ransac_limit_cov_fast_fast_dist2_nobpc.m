...
if check_stored_visual_feature(data_name, dm, cframe, sequence_data, image_name) == 0
    ...
    if strcmp(data_name, 'kinect_tum')
        ...
        [img1, x1, y1, z1, c1, elapsed_pre] = load_or_datasets(data_name, filter_name, boarder_cut_off, dm, cframe, scale, value_type);
    end
...
if check_stored_visual_feature(data_name, dm, cframe, sequence_data, image_name) == 0
    ...
    if strcmp(data_name, 'kinect_tum')
        ...
        [img2, x2, y2, z2, c2, elapsed_pre2] = load_or_datasets(data_name, filter_name, boarder_cut_off, dm, cframe, scale, value_type);
    end
    if strcmp(image_name, 'depth') %image_name == 'depth'
        %Assign depth image to img1
