...
%% Add additional odometry
for i=nextPoseIndex:-1:nextPoseIndex-3
    if i >= 2
        odometry = truth.cameras{i-1}.pose.between(truth.cameras{nextPoseIndex}.pose);
        newFactors.add(BetweenFactorPose3(symbol('x',i-1), symbol('x',nextPoseIndex), odometry, noiseModels.odometry));
    end
end
