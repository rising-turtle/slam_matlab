function [noiseModels,isam,result,nextPoseIndex] = VisualISAMInitialize_VRO(data,truth,options)
...
import gtsam.*
noiseModels.pose = noiseModel.Diagonal.Sigmas([0.034; 0.034; 0.034; 0.5*pi/180; 0.5*pi/180; 0.5*pi/180]);  % [m][radian]
...
noiseModels.odometry = noiseModel.Diagonal.Sigmas([0.034; 0.034; 0.034; 0.5*pi/180; 0.5*pi/180; 0.5*pi/180]);  % [m][radian]
%noiseModels.point = noiseModel.Isotropic.Sigma(3, 0.1);
%noiseModels.measurement = noiseModel.Isotropic.Sigma(2, 1.0);
noiseModels.point = noiseModel.Isotropic.Sigma(3, 0.034);
noiseModels.measurement = noiseModel.Isotropic.Sigma(2, 1.0);
