...
MAX_FRAME = 1000; %2000;
nFrame = 500; %540; %1000; %240; %60; %norminal frame
rFrame = 0;  %real frame
r2d=180.0/pi;
scale = 1;

image_name_list={'intensity','depth','fuse','fuse_std'};
data_name_list={'pitch', 'pan', 'roll','x2', 'y2', 'c1', 'c2','c3','c4','m','etas','loops2','kinect_tum','sparse_feature','loops','swing','swing2','object_recognition','motive','primesense'}; % 'x', 'y', 
...
data_name=data_name_list{20};  % 11 = 'etas', 12 = 'loops2', 13='kinect_tum', 14 = 'sparse_feature', 15='loops', 16='swing', 17='swing2', 18='object_recognition', 19='motive', 20='creative
...
consecutive_error_count_threshold = 0; %190;
...
switch data_name
    case {'primesense'}
        maxData = 10; % 11
        inc_max = 5; %9; %9; %11;   % 3 * 11 = 33 << 34 degree of FOV
        unit='degree';
        interval = angle_interval;
        sequence_data = false;
        start_frame_interval=0;
    case {'pitch','pitch2'}
        maxData = 28; % 11
    ...
    for k=1:maxDegree
        ...
        switch data_name
            ...
            otherwise
                ...
                feature_points_file_name = sprintf('result/%s/%d_%s/%d_frame_abs_%s_%s_%s_%d_feature_points.dat',data_name,inc*interval,unit,rFrame,image_name,fe_name,filter_name,boarder_cut_off);
...
%                 end
                ...
                switch fe_name
                 ...
                 case {'sift_i_r_s_i_t_t_c_i_a_c_c_featureidxfix_fast_fast_dist2_min40_histeq','sift_i_r_s_i_t_t_c_i_a_c_c_featureidxfix_fast_fast_dist2_min40_lc','sift_i_r_s_i_t_t_c_i_a_c_c_featureidxfix_fast_fast_dist2_min40', 'sift_i_r_s_i_t_t_c_i_a_c_c_featureidxfix_fast_fast_dist2_10step'}
                    ...
                    % Creative
                    case {'sift_i_r_s_i_t_t_c_i_a_c_c_featureidxfix_fast_fast_dist2_nobpc_primesense'}
                    %case {'sift_i_r_s_i_t_t_c_i_a_c_c_featureidxfix_fast_fast_dist2_nobpc_c'}
                        [phi(j), theta(j), psi(j), trans(:,j), error(j), elapsed(:,j), match_num(:,j), feature_points{j}, pose_std(:,j)]=localization_sift_ransac_limit_cov_fast_fast_dist2_nobpc_pm1(image_name, data_name, filter_name, boarder_cut_off, ransac_iteration_limit, valid_ransac_num, scale, value_type, k, inc, next_frame, sframe, sequence_data, is_10M_data, feature_dis);               
