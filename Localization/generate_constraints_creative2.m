...
MAX_FRAME = 1000; %2000;
nFrame = 500; %1000; %240; %60; %norminal frame
rFrame = 0;  %real frame
r2d=180.0/pi;
scale = 1;

image_name_list={'intensity','depth','fuse','fuse_std'};
data_name_list={'creative','pitch', 'pan', 'roll','x2', 'y2', 'c1', 'c2','c3','c4','m','etas','loops2','kinect_tum','sparse_feature','loops','swing','swing2','object_recognition','motive'}; % 'x', 'y', 
...
data_name=data_name_list{1};  % 11 = 'etas', 12 = 'loops2', 13='kinect_tum', 14 = 'sparse_feature', 15='loops', 16='swing', 17='swing2', 18='object_recognition', 19='motive'
...
%                 end
                ...
                switch fe_name
                    ...
                    %creative
                    case {'sift_i_r_s_i_t_t_c_i_a_c_c_featureidxfix_fast_fast_dist2_nobpc_c2'}% add  _c2
                       % [phi(j), theta(j), psi(j), trans(:,j), error(j), elapsed(:,j), match_num(:,j), feature_points{j}, pose_std(:,j)]=localization_sift_ransac_limit_cov_fast_fast_dist2_nobpc(image_name, data_name, filter_name, boarder_cut_off, ransac_iteration_limit, valid_ransac_num, scale, value_type, k, inc, next_frame, sframe, sequence_data, is_10M_data, feature_dis);
                         [phi(j), theta(j), psi(j), trans(:,j), error(j), elapsed(:,j), match_num(:,j), feature_points{j}, pose_std(:,j)]=localization_sift_ransac_limit_cov_fast_fast_dist2_nobpc_c2(image_name, data_name, filter_name, boarder_cut_off, ransac_iteration_limit, valid_ransac_num, scale, value_type, k, inc, next_frame, sframe, sequence_data, is_10M_data, feature_dis);
