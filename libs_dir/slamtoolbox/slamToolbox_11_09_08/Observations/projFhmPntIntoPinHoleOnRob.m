function [u,s,U_r,U_s,U_pk,U_pd,U_l]  = projFhmPntIntoPinHoleOnRob(Rf, Sf, Spk, Spd, l)

% PROJFHMPNTINTOPINHOLEONROB Project Fhm pnt into pinhole on robot.
%    [U,S] = PROJFHMPNTINTOPINHOLEONROB(RF, SF, SPK, SPD, L) projects 3D
%    framed homogeneous points into a pin-hole camera mounted on a robot,
...
%       L  : 3D framed homog. point [x y z a b c d vx vy vz rho]'
%    The output parameters are:
%       U  : 2D pixel [u v]'
%       S  : non-measurable depth
%
%    The function accepts an fhm points matrix L = [L1 ... Ln] as input.
...
if nargout <= 2  % No Jacobians requested
    ...
    if size(l,2) == 1
        ...
        error('??? Jacobians not available for multiple FHM points.')
