function [euler] = plane_fit_to_data_single(data_name) %plane_fit_to_data(idx,varargin)
%global myCONFIG
%%% tets ransac plane fitting
% clear all
% close all
% clc
%[x_sr,y_sr,z_sr,im] = read_xyz_sr4000_test(myCONFIG.PATH.DATA_FOLDER,idx);

% addpath('D:\Soonhac\SW\plane_fitting_code\all');
% addpath('D:\Soonhac\SW\slamtoolbox\slamToolbox_11_09_08\FrameTransforms\Rotations');
% addpath('D:\Soonhac\SW\Localization');
% data_name =  'motive'; %'object_recognition'; %'motive'; %'sparse_feature'; % % %'loops'; %'loops2'; %; %'loops2'; %'etas'; %'loops';
% dir_index = 18; %10; %3; %3;
% file_index = 1;

[frm1, des1, elapsed_sift, img1, x1, y1, z1, c1, elapsed_pre] = load_visual_features(data_name, 1, 1, true, 'intensity');
...
%[im, x_sr, y_sr, z_sr, c, rtime] = LoadSR(data_name, 'gaussian', 0, dir_index, file_index, 1, 'int'); 
...
%x = -x_sr; y = z_sr; z = y_sr; %%% transform it into camera coordinate
